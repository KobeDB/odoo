...
class SaleOrderCommunication:
    ...
    def _notify_get_recipients_groups(self, groups, msg_vals=None):
        ...
        try:
            ...
            is_tx_pending = self.order.get_portal_last_transaction().state == 'pending'
    ...
    def _track_subtype(self, init_values):
        self.order.ensure_one()
        if 'state' in init_values and self.order.state == 'sale':
            return self.env.ref('sale.mt_order_confirmed')
        elif 'state' in init_values and self.order.state == 'sent':
